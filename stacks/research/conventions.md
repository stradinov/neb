...
| **Voz externa (LLM)** | **Gemini** (modelo propietario de Google) | **Solo en research `propósito = exploratorio`**, inline en el mismo turno que el `WebSearch` de Claude — segunda voz independiente para ampliar cobertura y reducir sesgo de modelo. No en `decisión`. Groq / Ollama / OpenRouter quedan fuera del stack |
