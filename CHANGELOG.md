...
- **Jerarquía de prioridad**: prompt > **banda curada** (`source='curated'`, sin compas ni intrínsecas) > `compas.md` (peso solo del eje tópico + bonus solo del eje cliente; sin doble conteo) > intrínsecas. Para un pendiente **sin curar** cuenta solo la **mejor sugerencia por eje** (por score del matching), no el máximo de todas las sugerencias: un texto largo matchea decenas de temas y cualquiera que rozara un tópico de peso alto subía a "alta" (visto en dogfooding). `triage` expone `suggested_cliente`/`suggested_topico` y acota `suggestions` a 3 por eje. `infer_objectives` propone solo temas activos fuera del eje cliente. Motivo de la jerarquía: una brújula por tema no reproduce un juicio por pendiente (techo medido ~65 %).
...
- **Matching**: las keywords y el nombre de un tema se puntúan sin stopwords ni tokens de menos de 3 letras (`_match_tokens`): una keyword multi-palabra como "aviso al cliente" aportaba `al`, `a`, `con`, que matchean cualquier texto y hacían ganar a ese tema en casi todos los pendientes (visto en dogfooding). El seed, cuando el catálogo cambia (keywords/nombres/ejes), borra las sugerencias no curadas para que el siguiente `triage` las recalcule — afinar keywords = editar `taxonomy.json` + `--apply`.
