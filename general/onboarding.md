...
Política transversal. Define el comportamiento de Claude al detectar un usuario nuevo y qué ofrece el tour de bienvenida disponible vía `/wakeup`. Los **pasos** del setup (instalar, montar overlay, definir stack) viven en [`../docs/user-guide.md`](../docs/user-guide.md); este archivo define **cuándo** Claude ofrece el tour y **qué opciones** presenta — no los pasos en sí.

## Trigger de oferta pasiva

**Condición**: el stack activo es `self-applied` (el cwd está en el repo de neb) Y el contexto de la sesión no incluye contenido de `personal/<usuario>.md` — sin sección de preferencias personales, atajos ni paths absolutos de la máquina del dev en el contexto cargado.
...
2. **Definir tu primer stack** — concreta Neb para el dominio; puede incluir skill de apoyo y agentes revisores (ver [user-guide § Definir tu primer stack](../docs/user-guide.md)).
