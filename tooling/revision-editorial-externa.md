...
- **No alucines problemas de estructura.** Si reportas que una tabla tiene líneas en blanco entre filas o una fila partida, confírmalo en el archivo real: en el primer pase se reportaron rupturas de tabla inexistentes. Un `[ACTUAL]` mangleado (con el `[PROPUESTO]`/`motivo` metidos dentro de la tabla) es señal de que normalizaste mal el original.
