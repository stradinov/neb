...
## Backend central — contrato, disparador y opt-out
...
- **Disparador determinista**: el cliente publica al central cuando hay `NEB_LOGBOOK_ENDPOINT` **y** el proyecto no tiene opt-out. **Opt-out por proyecto**: marcador `<!-- neb-logbook: local -->` en el `CLAUDE.md` (lectura barata antes de spawnear el sync). *(Opt-out por perfil: futuro.)*
