...
| Cierre | Completar métricas y marcar `Cerrado` (ver [`workflow/index.md`](index.md) "Estados del requerimiento"). Finaliza el registro **en el mismo commit que entrega el entregable**, con `**Commits:** esta confirmación` (sin hash literal: no existe al redactar, y un cierre `.md`-only posterior dispara el gate de fragment del pre-push — ver [`../process/version-control.md`](../process/version-control.md) § "CHANGELOG fragmentado"). Si el registro ya se entregó antes (entrega temprana), su cierre va en un commit `docs: ...` aparte |

**Entrega temprana del registro.** El **registro del requerimiento** (su change MD — ver [`../methodology/vocabulary.md`](../methodology/vocabulary.md) § "Registro del requerimiento") es un **artefacto que Neb genera** (y, derivadamente, `.md`), por lo que confirmarlo y entregarlo es **autónomo** desde su creación (Fase 4) bajo el ownership — no requiere OK del dev cuando el delta es solo `.md` (ver [`change-control-policy.md`](../methodology/change-control-policy.md) § "Ownership de archivos `.md`" y [`version-control.md`](../process/version-control.md) § Push). La entrega temprana (confirmar + publicar el registro **desacoplada** de la entrega del entregable) aplica **cuando el entorno de validación es compartido** — **disparador determinista** (desde el backend central del logbook): hay `NEB_LOGBOOK_ENDPOINT` y el proyecto declaró opt-in (marcador `<!-- neb-logbook: central -->`), por lo que el `work` se publica a la bitácora compartida; publicar el `work` para relevo cross-dev exige que su registro esté **entregado** para que el puntero resuelva en la máquina que releva (ver [`logbook.md`](logbook.md) § "Entorno compartido"). Si el entorno **no** es compartido (sesión solo del dev, sin bitácora compartida), el registro se confirma/entrega al cierre. Esto cubre solo el **registro**: el entregable (los `.md` de la metodología misma) mantiene su propio gate de validación/cierre.
