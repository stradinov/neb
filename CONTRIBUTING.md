...
- **💡 Propuesta de mejora** — una fricción, una brecha o una idea para mejorar la metodología o sus herramientas.
...
Cada plantilla es un formulario con campos guiados; al llenarlos, tu issue cumple el esquema de descripción que el equipo necesita para clasificarlo sin ida y vuelta.
...
El formulario te pide estos elementos; respetarlos acelera la clasificación:
...
- **Cuida la evidencia.** Adjunta transcripciones, mensajes de error o capturas, pero **sin información personal (PII), datos de clientes, marcas ni rutas absolutas de tu máquina**. Neb es agnóstica de dominio: usa ejemplos genéricos.

## Qué pasa después

El equipo mantenedor clasifica el issue. Si se acepta, se convierte en un requerimiento que sigue el flujo `self-applied` de Neb: se clarifica el alcance, se propone, se implementa con revisión de roles, y se entrega con su entrada en el `CHANGELOG.md` y su incremento de versión. El issue queda enlazado al cambio para trazabilidad.

## Para mantenedores

Las convenciones de edición del repo (convención de nombres `kebab-case`, `@imports` relativos, versionado SemVer, flujo de `CHANGELOG.md` fragmentado, propiedad de los archivos `.md`) viven en [`CLAUDE.md`](CLAUDE.md) y en la metodología misma.
