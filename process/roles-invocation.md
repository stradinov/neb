...
**Nota sobre nombres de rol**: los roles "Process Architect", "QA Process Engineer" y "Process Improvement Analyst" (y los subagentes `qa-process-engineer`, `process-improvement-analyst`) conservan la palabra "Process" en su nombre. Aquí "Process" se refiere al concepto general de **proceso de metodología** (Process Architect = arquitecto del proceso metodológico), no al nombre actual del stack. La etiqueta sobrevive al rename del stack por inercia conceptual; el rename de roles se evalúa en REQ separado si aparece fricción real.

## Selección de roles por contexto

Claude propone los roles en Fase 3 (Propuesta) antes de generar el plan inicial. Para casos rutinarios usa default sin preguntar; para casos especiales (stack desconocido, tipo REQ raro, plan toca dimensión nueva) pregunta al dev.

### Default por stack

| Stack | Rol principal | Revisores default | Detalle |
|---|---|---|---|
| `self-applied` | Process Architect | QA Process Engineer + Process Improvement Analyst | [stacks/self-applied/roles.md](../stacks/self-applied/roles.md) |
...
|---|---|---|
| **Silencioso** | Asigna y procede sin notificar. Dev ve los roles al revisar el MD | Patches triviales (typos, fixes de enlaces, cosméticos); stack `self-applied` con roles fijos default y sin extras; repetición exacta del REQ anterior en la misma sesión |
| **Verbose** | Anuncia inline (*"Plan toca [X, Y]. Roles: principal `<rol>`, revisores `<rol>` + `<rol>`. Procedo."*); procede salvo objeción | Casos rutinarios en stack de software con default + 0–2 revisores totales |
...
| **Bloqueante** | Pregunta explícitamente *"¿OK con estos roles?"* y espera respuesta del dev | Stack desconocido; archivo no matchea dimensión y default no aplica claro; conflicto entre dimensiones (ej. `index.php` con auth: routing crítico vs auth); REQ tipo poco común (security review profundo, migración masiva, hotfix de seguridad); dimensión nueva no listada en el catálogo; override solicitado por el dev |
...
Ejemplo: REQ con stack desconocido y solo 1 revisor (verbose por revisores; bloqueante por stack) → **bloqueante**.

La tabla puede expandirse vía Fase 9 si surge un caso recurrente no cubierto.

## Cobertura mínima por fase

**Regla**: cada stack debe tener ≥1 subagente formalizado activo en Fase 4 (cierre/gate) y Fase 7 (pre-ejecución/gate). Si el stack no tiene subagente disponible, el dev debe declarar override explícito antes de avanzar. Las fases 1, 2, 6, 8 son conducidas únicamente por el rol principal (persona) — sin gate de subagente.

### Matriz de cobertura por stack × fase (subagentes específicos del stack)

Las fases 1, 2, 6, 8 se omiten (nunca tienen subagente). `—` = fase conducida solo por persona. Esta matriz lista únicamente los subagentes **específicos del stack**; el revisor transversal `context-completeness-reviewer` se invoca adicionalmente en F3/F4/F7 según las condiciones de [`plan-review.md`](plan-review.md) § "Cuándo aplica el `context-completeness-reviewer`".

| Stack | F3 Plan-review | F4 Cierre (gate) | F5 Validación | F7 Pre-ejecución (gate) | F9 Diagnóstico | Incidentes P1/P2 |
|---|---|---|---|---|---|---|
...
Adicionalmente, `context-completeness-reviewer` se invoca de forma **transversal** (todos los stacks) en F3/F4/F7 según las condiciones de [`plan-review.md`](plan-review.md) § "Cuándo aplica el `context-completeness-reviewer`". No reemplaza a los revisores específicos del stack — los complementa auditando suposiciones que el autor no verificó (estado concreto local o dominio sin antecedente).
